...
#| apa-note: "Income levels were designated by MEPS guidelines"
#| fig.width: 12
#| fig.height: 8
#| echo: false
...
fig_2 <- ggplot(clean_data2, aes(x = INSURC22, fill = as.factor(exposure_psych_drugs))) +
  ...
  theme(axis.title.y = element_text(margin = margin(r = 10), size=12), # Add space between y-axis label and plot
    axis.title.x = element_text(margin = margin(t = 15), size=12)) +  # Add space between x-axis label and plot) 
  theme_minimal()
...
ggplot(clean_data2, aes(x = AGE31X, y = psych_conditions_count, color = SEX)) +  
  ...
  theme(
    ...
    axis.text.y = element_text(size = 10),  # Adjust y-axis text
    axis.title.y = element_text(margin = margin(r = 15), size = 12),  # Add space between y-axis label and plot
    axis.title.x = element_text(margin = margin(r = 10), size = 12)  # Add space between x-axis label and plot
  )
```
@fig-age-psychcond displays the relationship between **age** and **number of psychiatric diagnoses**. 

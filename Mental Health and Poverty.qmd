...
my_data <- my_data %>%
  mutate(
    RACETHX = as.factor(RACETHX),   # Race
...
table(my_data$RACETHX)
table(my_data$INSURC22)
table(my_data$psych_drug_exposure)
table(my_data$psych_cond_exist)
...
# Mutate psych_drug_exposure to a factor
all_merged <- all_merged %>%
  mutate(psych_drug_exposure = factor(psych_drug_exposure, levels = c("No", "Yes")))
...
table1 <- all_merged %>%
  ...
  summarise(
    ...
    `Income: Poor/Negative (%)` = paste0(round(mean(POVCAT22 == "Poor/Negative", na.rm = TRUE) * 100, 2), "%"),
    `Income: Near Poor (%)` = paste0(round(mean(POVCAT22 == "Near Poor", na.rm = TRUE) * 100, 2), "%"),
    `Income: Low Income (%)` = paste0(round(mean(POVCAT22 == "Low Income", na.rm = TRUE) * 100, 2), "%"),
    `Income: Middle Income (%)` = paste0(round(mean(POVCAT22 == "Middle Income", na.rm = TRUE) * 100, 2), "%"),
    `Income: High Income (%)` = paste0(round(mean(POVCAT22 == "High Income", na.rm = TRUE) * 100, 2), "%")
  ) %>%
  pivot_longer(cols = -psych_drug_exposure, names_to = "Variable", values_to = "Value") %>%
  pivot_wider(names_from = psych_drug_exposure, values_from = Value)
